function createDataset(fields, constraints, sortFields) {
	...
	if (fields != null && fields != "") {
		...
		for (var i = 0; i < totalUsers.length; i++) {
        	...
        	constraints.push( DatasetFactory.createConstraint("matriculaUsuario", totalUsers[i], totalUsers[i], ConstraintType.SHOULD) );
        }
	}
	...
	for (var i = 0; i < dtsRelatorio.values.length; i++) {
		...
		if (fields != null && fields != "") {
			...
			Adicionar_linha = CompareDate(dtsRelatorio.getValue(i, "dtRegistro"), fields[1], fields[2]);
		}

		if (Adicionar_linha) {
			dataset_Auxiliar.push( dtsRelatorio.getValue(i, "nmUsuario") );
		}
	}
